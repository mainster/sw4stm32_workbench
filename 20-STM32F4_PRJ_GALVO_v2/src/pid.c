...
#define LARGE_PID
    ...
    /* Calculate integral term and limit integral runaway. */
    if (eSum_1 > pid->maxSumError) {
        y_i = MAX_I_TERM;
        eSum_1 = pid->maxSumError;
    }
    ...
    // y_d = Kd/Ts * (e - e_1);                             //<! Without 1. order filter.
    y_d = Kd / Ts * (e - e_1) / ((e - e_1) * Tf / Ts + 1);  //<! With 1. order filter.

    y = (y_p + y_i + y_d);                                  //<! Sum all terms.
    if (y > UPPER_DAC_LIMIT_SIGNED) {
        y = UPPER_DAC_LIMIT_SIGNED;
    }
    else if (y < LOWER_DAC_LIMIT_SIGNED) {
        y = LOWER_DAC_LIMIT_SIGNED;
    }
